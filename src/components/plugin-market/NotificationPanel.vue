  ...
  <div class="notification-panel">
    <div class="card panel-card panel-hero">
      ...
      <div class="hero-actions">
        <button class="btn btn-md btn-ghost" type="button" :disabled="loading" @click="emit('refresh')">
          刷新
        </button>
        <button
    ...
    <template v-if="!currentUser">
      <div class="card panel-card section-card empty-card">
        ...
        <div class="login-cta">
          <button class="btn btn-md btn-primary" type="button" @click="emit('go-login')">前往登录</button>
        </div>
      </div>
    </template>
    ...
    <template v-else>
      <div class="card panel-card section-card">
        ...
        <div v-else-if="error" class="error-container">
          <span>{{ error }}</span>
          <button class="btn btn-md btn-ghost" type="button" @click="emit('refresh')">重试</button>
        </div>
        ...
        <div v-if="shouldShowPagination" class="pagination-row">
          <button class="btn btn-md btn-ghost" type="button" :disabled="!canGoPrev" @click="emit('change-page', page - 1)">
            上一页
          </button>
          <span class="pagination-text">共 {{ total }} 条，当前第 {{ page }} / {{ totalPages }} 页</span>
          <button class="btn btn-md btn-ghost" type="button" :disabled="!canGoNext" @click="emit('change-page', page + 1)">
            下一页
          </button>
        </div>
      </div>
